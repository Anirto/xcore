...
struct request *parse_request(const char *str)
{
    ...
    char *name = NULL;
    char *value = NULL;
    enum rp release_policy = rel_both;

    /* 解析请求行 */
    line = strtok(xstrdup(str), "\r\n");
    method = strtok(line, " ");
    arg = strtok(NULL, " ");
    request_set_method(req, method, arg);

    /* 解析请求头 */
    while ((line = strtok(NULL, "\r\n"))) {
        name = strtok(line, ":");
        value = strtok(NULL, "");
        release_policy = rel_both;
        if (name && value) {
            request_set_header(req, name, value, release_policy);
        }
    }
